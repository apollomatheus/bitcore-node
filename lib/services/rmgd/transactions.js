  ...
  function addInputsToTx(tx, result, transactions) {
    ...
    for(var inputIndex = 0; inputIndex < result.vin.length; inputIndex++) {
      ...
      if (inputTx) {
        ...
        // Admin transactions doesn't have this
        if (correspondingVout && correspondingVout.scriptPubKey && correspondingVout.scriptPubKey.addresses && correspondingVout.scriptPubKey.addresses.length > 0) {
    ...
    client.getRawTransaction(txid, 1, function(err, response) {
      ...
      async.mapSeries(
        ...
        function(err, transactions) {
          ...
          // Admin tx's doesn't have inputs, resulting a negative value for fees.
          if (!tx.coinbase && tx.inputSatoshis > 0) {
...
var latestTransactionsPool = [];
var MAX_TX_POOL_SIZE = 25; // 25 To have room for fetching more, currently UI will fetch only 5
/**
 * We are going to have in memory the latest transactions that are in broadcasted by the node
 * This list will allow to fetch the latest transactions, since we are going to have only a few txs we can have this
 * in memory.
 * @param transaction
 */
var addTransactionToLatestPool = function(transaction) {
  latestTransactionsPool.unshift(transaction);
  // Let's keep this array of a fixed size
  if(latestTransactionsPool.length > MAX_TX_POOL_SIZE) {
    latestTransactionsPool.pop();
  }
};
...
module.exports = {
  ...
  addTransactionToLatestPool: addTransactionToLatestPool,

  latestTransactionsPool: latestTransactionsPool
};