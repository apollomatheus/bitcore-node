  ...
  self.getAddressTxids(addresses, { queryMempool: false }, function(err, txids) {
    async.mapSeries(
      ...
      function(err, transactions) {
        ...
        var balanceSummary = summarizeTxBalance(transactions, addresses);
        callback(null, balanceSummary);
      }
    );
  });
};
...
/**
 * Get a list of detailed transactions, and summarize his inputs and outputs to get the
 * address balance.
 *
 * Receive is the sum of satoshis where the addresses we are looking for is involved in the outputs.
 * Sends is the sum of inputs where we send from the addresses we are looking for
 *
 * @param transactions
 * @param addresses
 * @returns {{balance: number, received: number}}
 */
var summarizeTxBalance = function (transactions, addresses) {
  var amountSent = 0, amountReceived = 0;
  transactions.forEach(function(tx) {
    // Inputs
    tx.inputs.forEach(function(input){
      if (input.address && _.indexOf(addresses, input.address) > -1) {
...
module.exports = {
  ...
  summarizeTxBalance: summarizeTxBalance
}