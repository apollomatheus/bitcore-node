 ...
 */
WebService.prototype.start = function(callback, options) {
  this.app = express();
  ...
  var redisHost = options.redishost || null;
  var redisPort = options.redisport || 6379;

  if(redisHost) {
    var pub = redis.createClient(redisPort, redisHost);
    var sub = redis.createClient(redisPort, redisHost, {detect_buffers: true});
...
WebService.prototype.setupAllRoutes = function() {
  ...
  // Add the /img as a static so it can be accessed by URL
  this.app.use('/img', express.static(path.join(__dirname, '../../../insight-ui/public/img')));
