...
Bitcoin.prototype.webSocketBlockHandler = function(node, message) {
  var self = this;
...
Bitcoin.prototype._subscribeWsEvents = function(ws) {
  var self = this;
  var msg = JSON.stringify({"jsonrpc":"1.0","method":"notifyblocks","params":[],"id":1});
  ws.send(msg);
};

Bitcoin.prototype._initWsSubSocket = function(node, config) {
  var self = this;
  var wsUrl = 'ws://' + config.rpclimituser + ':' + config.rpclimitpass + '@' + config.rpchost + ':' + config.rpcport + '/ws';
